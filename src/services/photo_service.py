...
class PhotoService:
    ...
    def get_photo_by_hash(self, hash: str) -> Photo | None:
        ...
        result = self.db.execute(statement=query).scalars().one_or_none()
        if result is None:
            self.logger.debug(f"No photo found with hash={hash}")
        return result
    ...
    def get_photo_by_id(self, id: int) -> Photo | None:
        ...
        result = self.db.execute(statement=query).scalar_one_or_none()
        if result is None:
            self.logger.debug(f"No photo found with id={id}")
        return result
    ...
    def get_photos_by_collection(self, collection_name: str) -> Sequence[Photo]:
        ...
        self.logger.debug(f"Found {len(results)} photos in collection={collection_name}")
        return results
    ...
    def get_hero_photo(self) -> str | None:
        ...
        result = self.db.execute(statement=query).scalar_one_or_none()
        if result is None:
            self.logger.warning("No hero photo found in database")
        return result
    ...
    def get_about_image(self) -> Photo | None:
        ...
        result = self.db.execute(statement=query).scalar_one_or_none()
        if result is None:
            self.logger.warning("No about_me photo found in database")
        return result
    ...
    def get_unique_collections(self) -> list[str]:
        ...
        results = self.db.execute(statement=query).scalars().all()
        self.logger.debug(f"Found {len(results)} unique collections")
        return list(results)
